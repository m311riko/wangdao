    ...
    while(1)
    {
        ...
        if(readyFd>0)
        {
            ...
            if(FD_ISSET(socketFd,&rdset))
            {
                ...
                strcpy(info[k].name,buf);

                //放入描述符集合
                FD_SET(new_fd,&tmpset);
                info[k].new_fd=new_fd;
                k++;
            }
            ...
            for(int i=0;i<k;i++)
            {
                if(FD_ISSET(info[i].new_fd,&rdset))
                {
                    bzero(buf,sizeof(buf));
                    ret=recv(info[i].new_fd,buf,sizeof(buf),0);
                    ...
                    for(int j=0;j<k;j++)
                    {
                        ...
                        strcat(msg,info[i].name);
                        strcat(msg,msg2);
                        strcat(msg,buf);
                        send(info[j].new_fd,msg,strlen(msg),0);
                    }
                }
            }
